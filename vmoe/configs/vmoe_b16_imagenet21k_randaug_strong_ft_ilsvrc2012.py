...
r"""Fine-tune gs://vmoe_checkpoints/vmoe_b16_imagenet21k_randaug_strong on ILSVRC2012.

Accuracy (mean over 3 runs with different fine-tuning seeds):
  - ILSVRC2012, validation: 89.2%
  - ILSVRC2012, test: 85.6%
  - ILSVRC2012 ReaL, test (mean over 3 runs): 84.1%
...
GROUP_SIZE = 4 * ((IMAGE_SIZE // PATCH_SIZE)**2 + 1)
...
def get_config():
  ...
  config.dataset.test_real = get_data_params(
