...
class ReportProgressTest(parameterized.TestCase):
  ...
  def test(self, available, expected):
    ...
    self.assertLen(call_arg_list, 3 * 2)
    self.assertEqual(call_arg_list[0],
                     mock.call(4, {'steps_per_sec': mock.ANY}))
    self.assertEqual(call_arg_list[1], mock.call(4, {'uptime': mock.ANY}))
    ...
    self.assertEqual(call_arg_list[4], mock.call(8, {'uptime': mock.ANY}))
